...
void Emscripten_RegisterEventHandlers(SDL_WindowData *data)
{
    ...
    if (keyElement) {
        ...
        // see registerOrRemoveHandler in library_html5.js. If a previous SDL window
        // already registered keyboard handlers on the same target, the new
        // registration would *stack* a second listener, causing every browser
        // keydown to fire Emscripten_HandleKey twice. The duplicate calls then
        // produce two SDL_EVENT_KEY_DOWN events per physical keypress (the
        // second one with repeat=true, courtesy of the keystate-based repeat
        // detection in SDL_SendKeyboardKeyInternal). Clear any prior handler
        // on this target before installing ours.
        ...
        MAIN_THREAD_EM_ASM_INT({
            ...
            // Remove any prior listener first — addEventListener does not
            // deduplicate, so re-registering without unregistering would
            // stack listeners and double-fire on every keydown.
